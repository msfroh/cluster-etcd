...
public class ZoneAntiAffinityDecider implements AllocationDecider {
    private static final Logger log = LogManager.getLogger(ZoneAntiAffinityDecider.class);
    ...
    public Decision canAllocate(String shardId, SearchUnit node, String indexName, NodeRole targetRole) {
        ...
        if (usedZones == null || usedZones.isEmpty()) {
            log.trace("ZoneAntiAffinity: No zones used yet, allowing node {} (zone={})", node.getName(), node.getZone());
            return Decision.YES;
        }

        // If node has no zone info, allow by default
        if (node.getZone() == null || node.getZone().isEmpty()) {
            log.debug("ZoneAntiAffinity: Node {} has no zone info, allowing by default", node.getName());
            return Decision.YES;
        }
        ...
        if (usedZones.contains(node.getZone())) {
            log.debug(
                "ZoneAntiAffinity: Node {} (zone={}) is in already-used zones {}, rejecting",
                node.getName(),
                node.getZone(),
                usedZones
            );
        ...
        log.trace("ZoneAntiAffinity: Node {} (zone={}) is in different zone from {}, accepting", node.getName(), node.getZone(), usedZones);
        return Decision.YES;
    }
