...
public class IngesterNodeSelector {
     ...
     */
    public SearchUnit selectNodeFromGroup(NodesGroup group, String shardId, String indexName, Set<String> usedZones) {
        List<SearchUnit> nodesInGroup = group.getNodes();

        if (nodesInGroup == null || nodesInGroup.isEmpty()) {
            log.warn("IngesterNodeSelector: Group {} has no nodes for shard {}/{}", group.getGroupId(), indexName, shardId);
            return null;
        }

        // Step 1: Apply HealthDecider
        HealthDecider healthDecider = new HealthDecider();
        List<SearchUnit> healthyNodes = filterNodesByDecider(nodesInGroup, shardId, indexName, healthDecider);

        if (healthyNodes.isEmpty()) {
            log.warn("IngesterNodeSelector: No healthy nodes in group {} for shard {}/{}", group.getGroupId(), indexName, shardId);
            return null;
        }

        log.debug(
            "IngesterNodeSelector: {}/{} nodes are healthy in group {}",
            healthyNodes.size(),
            nodesInGroup.size(),
            group.getGroupId()
        );
        ...
        List<SearchUnit> nodesInDifferentZone = filterNodesByDecider(healthyNodes, shardId, indexName, zoneDecider);
        ...
        if (!nodesInDifferentZone.isEmpty()) {
            ...
            if (usedZones != null && !usedZones.isEmpty()) {
                log.debug(
                    "IngesterNodeSelector: Zone anti-affinity succeeded - {}/{} nodes in different zones",
                    eligibleNodes.size(),
                    healthyNodes.size()
                );
            }
            ...
            log.warn(
                "IngesterNodeSelector: Zone anti-affinity failed for group {} - "
        ...
        SearchUnit selectedNode = selectionStrategy.selectNode(eligibleNodes, group, shardId, indexName);

        if (selectedNode == null) {
            log.error(
                "IngesterNodeSelector: Selection strategy returned null for group {} shard {}/{}",
                group.getGroupId(),
                indexName,
                shardId
            );
     ...
     */
    private List<SearchUnit> filterNodesByDecider(List<SearchUnit> nodes, String shardId, String indexName, AllocationDecider decider) {
