...
import org.opensearch.cluster.controller.models.Alias;
import org.opensearch.cluster.controller.models.CoordinatorGoalState;
import org.opensearch.cluster.controller.models.Index;
import org.opensearch.cluster.controller.models.IndexSettings;
import org.opensearch.cluster.controller.models.SearchUnit;
import org.opensearch.cluster.controller.models.SearchUnitActualState;
import org.opensearch.cluster.controller.models.SearchUnitGoalState;
...
import static org.opensearch.cluster.controller.metrics.MetricsConstants.UPDATE_ACTUAL_ALLOCATION_FAILURES_METRIC_NAME;
import static org.opensearch.cluster.controller.metrics.MetricsUtils.buildMetricsTags;
...
public class ActualAllocationUpdater {
    private static final Logger log = LogManager.getLogger(ActualAllocationUpdater.class);
    ...
    public void updateActualAllocations(String clusterId) throws Exception {
        ...
        log.info(
            "ActualAllocationUpdater - Completed actual allocation update with {} updates and {} coordinator goal state updates",
            totalUpdates,
            coordinatorUpdates
        );
    }
        ...
        for (SearchUnit searchUnit : searchUnits) {
            ...
            try {
                ...
                if (!Constants.ADMIN_STATE_NORMAL.equalsIgnoreCase(String.valueOf(searchUnit.getStateAdmin()))) {
                    log.debug(
                        "ActualAllocationUpdater - Skipping non-normal SU: {} (admin_state: {})",
                        unitName,
                        searchUnit.getStateAdmin()
                    );
                    continue;
                }
                        ...
                        if (shards != null) {
                            for (SearchUnitActualState.ShardRoutingInfo shard : shards) {
                                ...
                                if (!ShardState.STARTED.equals(shard.getState())) {
                                    log.debug(
                                        "ActualAllocationUpdater - Skipping non-STARTED shard {}/{} on SU: {} (state: {})",
                                        indexName,
                                        shard.getShardId(),
                                        unitName,
                                        shard.getState()
                                ...
                                actualAllocations.computeIfAbsent(indexName, k -> new HashMap<>())
                                    .computeIfAbsent(shardId, k -> new HashSet<>())
                                    .add(unitName);

                                log.debug(
                                    "ActualAllocationUpdater - Found STARTED shard {}/{} (primary: {}) on SU: {}",
                                    indexName,
                                    shardId,
                                    shard.isPrimary(),
                                    unitName
        ...
        for (Map.Entry<String, Map<String, Set<String>>> indexEntry : actualAllocations.entrySet()) {
            ...
            for (Map.Entry<String, Set<String>> shardEntry : indexEntry.getValue().entrySet()) {
                ...
                try {
                    ...
                    log.error(
                        "ActualAllocationUpdater - Error updating actual allocation for {}/{}: {}",
                        indexName,
                        shardId,
                        e.getMessage(),
                        e
                    );
                    metricsProvider.counter(UPDATE_ACTUAL_ALLOCATION_FAILURES_METRIC_NAME, buildMetricsTags(clusterId, indexName, shardId))
                        .increment();
     ...
     */
    private boolean updateShardActualAllocation(String clusterId, String indexName, String shardId, Set<String> allocatedUnits)
        throws Exception {
        ...
        for (String unitName : allocatedUnits) {
            try {
                ...
                if (searchUnit != null) {
                    String role = searchUnit.getRole() != null ? searchUnit.getRole().toUpperCase(Locale.ROOT) : "UNKNOWN";
        ...
        log.info(
            "ActualAllocationUpdater - Updated actual allocation for {}/{}: ingest={}, search={}",
            indexName,
            shardId,
            ingestSUs,
            searchSUs
     ...
     */
    private void cleanupStaleActualAllocations(String clusterId, Map<String, Map<String, Set<String>>> currentActualAllocations)
        throws Exception {
        ...
        Set<String> existingIndexNames = indexConfigs.stream().map(Index::getIndexName).collect(java.util.stream.Collectors.toSet());
        ...
        for (Index indexConfig : indexConfigs) {
            ...
            try {
                ...
                log.error("ActualAllocationUpdater - Error cleaning up actual allocations for index {}: {}", indexName, e.getMessage(), e);
            ...
            if (!existingIndexNames.contains(indexName)) {
                ...
                try {
                    ...
                    for (ShardAllocation allocation : orphanedAllocations) {
                        ...
                        log.info(
                            "ActualAllocationUpdater - Deleted orphaned actual allocation for deleted index {}/{}",
                            indexName,
                            shardId
                        );
                    }
                    ...
                    log.error(
                        "ActualAllocationUpdater - Error cleaning up orphaned actual allocations for deleted index {}: {}",
                        indexName,
                        e.getMessage(),
                        e
                    );
    ...
    private boolean isCoordinatorNode(SearchUnit unit) {
        ...
        String role = unit.getRole() != null ? unit.getRole().toLowerCase(Locale.ROOT) : "";
        if ("coordinator".equals(role)) {
            return true;
        }

        // Check if name starts with "coordinator" prefix (for data nodes acting as coordinators)
        String name = unit.getName() != null ? unit.getName().toLowerCase(Locale.ROOT) : "";
    ...
    public int updateCoordinatorGoalStates(String clusterId, List<SearchUnit> searchUnits) throws Exception {
        ...
        for (Index indexConfig : indexConfigs) {
            ...
            log.debug(
                "ActualAllocationUpdater - Processing coordinator routing for index '{}' with {} actual allocations",
                indexName,
                actualAllocations.size()
            );
            ...
            for (int shardIndex = 0; shardIndex < shardReplicaCount.size(); shardIndex++) {
                ...
                for (String unitName : actual.getIngestSUs()) {
                    if (isNodeGoalStateConverged(clusterId, unitName, indexName, shardId)) {
                        ...
                        shardNodes.add(node);
                        log.debug(
                            "ActualAllocationUpdater - Including primary node '{}' in coordinator routing for {}/{}",
                            unitName,
                            indexName,
                            shardId
                        );
                        ...
                        log.debug(
                            "ActualAllocationUpdater - Excluding primary node '{}' from coordinator routing for {}/{} (not converged)",
                            unitName,
                            indexName,
                            shardId
                        );
                ...
                for (String unitName : actual.getSearchSUs()) {
                    if (isNodeGoalStateConverged(clusterId, unitName, indexName, shardId)) {
                        ...
                        shardNodes.add(node);
                        log.debug(
                            "ActualAllocationUpdater - Including search replica node '{}' in coordinator routing for {}/{}",
                            unitName,
                            indexName,
                            shardId
                        );
                        ...
                        log.debug(
                            "ActualAllocationUpdater - Excluding search replica node '{}' from coordinator routing for {}/{} (not converged)",
                            unitName,
                            indexName,
                            shardId
                        );
        ...
        try {
            ...
            log.info("ActualAllocationUpdater - Updated coordinator goal state with {} indexes: {}", indices.size(), indices.keySet());
        } catch (Exception e) {
        ...
        log.info(
            "ActualAllocationUpdater - Completed coordinator goal state updates: {} shard updates across {} indexes with actual allocations: {}",
            totalUpdates,
            indexesWithActualAllocations.size(),
            indexesWithActualAllocations
        );
        return totalUpdates;
    }
    ...
    private boolean isNodeGoalStateConverged(String clusterId, String unitName, String indexName, String shardId) {
        try {
            ...
            if (goalState == null) {
                log.debug(
                    "ActualAllocationUpdater - Node '{}' has no goal state, considering not converged for {}/{}",
                    unitName,
                    indexName,
                    shardId
                );
            ...
            if (!goalState.hasIndex(indexName)) {
                log.debug("ActualAllocationUpdater - Node '{}' goal state doesn't include index '{}', not converged", unitName, indexName);
                return false;
            }
            ...
            log.debug(
                "ActualAllocationUpdater - Node '{}' goal state for index '{}': shards={}, contains {}? {}",
                unitName,
                indexName,
                goalShards,
                shardId,
     ...
     */
    private ShardAllocation getAggregatedActualAllocation(
        String clusterId,
        String indexName,
        String shardId,
    ...
    private boolean isNodeTimestampRecent(SearchUnitActualState actualState, String unitName) {
        ...
        if (timeDiff > ONE_MINUTE_MS) {
            log.debug("ActualAllocationUpdater - Skipping stale SU: {} (timestamp: {}, age: {}ms)", unitName, nodeTimestamp, timeDiff);
            return false;
        }
