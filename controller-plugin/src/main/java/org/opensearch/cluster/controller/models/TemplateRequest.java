...
public class TemplateRequest implements ToXContentObject {
    ...
    public TemplateRequest(List<String> indexPatterns, Integer priority, TemplateDefinition template, 
                          String instanceName, String region) {
                ...
                if ("index_patterns".equals(currentFieldName)) {
                    ...
                    while (parser.currentToken() != XContentParser.Token.END_ARRAY) {
                        if (parser.currentToken() == XContentParser.Token.VALUE_STRING) {
                            ...
                            throw new ParsingException(parser.getTokenLocation(), "Unexpected value " + parser.currentToken() + "for field " + currentFieldName);
                        }
                        parser.nextToken();
                    }
            ...
            while (parser.currentToken() != XContentParser.Token.END_OBJECT) {
                if (parser.currentToken() == XContentParser.Token.FIELD_NAME) {
                    ...
                    switch (currentFieldName) {
                        ...
                        case null, default ->
                                throw new ParsingException(parser.getTokenLocation(), "Unexpected map for field " + currentFieldName);
                    }
                } else {
                    throw new ParsingException(parser.getTokenLocation(), "Unexpected token " + parser.currentToken());
                }
