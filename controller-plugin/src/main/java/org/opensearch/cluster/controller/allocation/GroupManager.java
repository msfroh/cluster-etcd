...
public class GroupManager {
    ...
    public List<NodesGroup> discoverGroups(List<SearchUnit> allNodes) {
        ...
        Map<String, List<SearchUnit>> groupedByGroupId = allNodes.stream().collect(Collectors.groupingBy(this::getGroupId));
        ...
        for (Map.Entry<String, List<SearchUnit>> entry : groupedByGroupId.entrySet()) {
            ...
            Set<String> rolesInGroup = nodesInGroup.stream().map(SearchUnit::getRole).collect(Collectors.toSet());

            if (rolesInGroup.size() > 1) {
                // FATAL: Mixed roles in a group - this is a configuration error
                String nodeNames = nodesInGroup.stream().map(n -> n.getName() + ":" + n.getRole()).collect(Collectors.joining(", "));

                log.error(
                    "FATAL: Mixed roles detected in group (groupId={}) - Roles: {}. "
            ...
            group.setNodeIds(nodesInGroup.stream().map(SearchUnit::getName).collect(Collectors.toList()));
            ...
            log.debug("Discovered group: groupId={}, {} nodes, role={}", groupId, nodesInGroup.size(), role);
        }

        log.info("Discovered {} groups from {} nodes", groups.size(), allNodes.size());
    ...
    public List<NodesGroup> filterGroupsByRole(List<NodesGroup> groups, NodeRole targetRole) {
        return groups.stream().filter(group -> matchesRole(group, targetRole)).collect(Collectors.toList());
    }
