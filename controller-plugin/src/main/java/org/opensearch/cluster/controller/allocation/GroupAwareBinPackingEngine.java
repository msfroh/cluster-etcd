...
public class GroupAwareBinPackingEngine implements AllocationDecisionEngine {
    private static final Logger log = LogManager.getLogger(GroupAwareBinPackingEngine.class);
    ...
    public GroupAwareBinPackingEngine() {
        ...
        IngesterNodeSelectionStrategy ingesterNodeSelectionStrategy = new RandomIngesterNodeSelectionStrategy();
    ...
    private List<SearchUnit> getPrimaryNodesForAllocation(
        ...
        if (currentIngestSUs.size() == desiredGroupCount && !currentIngestSUs.isEmpty()) {
            log.info(
                "PRIMARY allocation for shard {}/{}: already has {} ingester(s) - checking stability",
                indexName,
                shardId,
                currentIngestSUs.size()
            );

            List<SearchUnit> existingNodes = validateExistingIngesters(currentIngestSUs, allNodes);
            if (existingNodes.size() == desiredGroupCount) {
                log.info("PRIMARY allocation for shard {}/{}: all ingesters healthy - stable allocation", indexName, shardId);
                return existingNodes;
            } else {
                log.warn("PRIMARY allocation for shard {}/{}: some ingesters unhealthy - reallocating", indexName, shardId);
            }
        }
        ...
        if (allGroups.isEmpty()) {
            log.error("PRIMARY allocation for shard {}/{}: no groups discovered. Allocation failed.", indexName, shardId);
            return List.of();
        }

        List<NodesGroup> primaryGroups = groupManager.filterGroupsByRole(allGroups, NodeRole.PRIMARY);
        if (primaryGroups.isEmpty()) {
            log.error("PRIMARY allocation for shard {}/{}: no PRIMARY groups found. Allocation failed.", indexName, shardId);
            return List.of();
        }

        log.debug("PRIMARY allocation for shard {}/{}: discovered {} PRIMARY groups", indexName, shardId, primaryGroups.size());

        // Step 3: Select N groups using RandomGroupSelectionStrategy
        List<NodesGroup> selectedGroups = groupManager.selectGroups(primaryGroups, NodeRole.PRIMARY, desiredGroupCount);

        if (selectedGroups.isEmpty()) {
            log.error("PRIMARY allocation for shard {}/{}: group selection failed. Allocation failed.", indexName, shardId);
            return List.of();
        }

        if (selectedGroups.size() < desiredGroupCount) {
            log.warn(
                "PRIMARY allocation for shard {}/{}: could only select {}/{} groups",
                indexName,
                shardId,
                selectedGroups.size(),
                desiredGroupCount
        ...
        log.info(
            "PRIMARY allocation for shard {}/{}: selected {} group(s) from {} available",
            indexName,
            shardId,
            selectedGroups.size(),
            primaryGroups.size()
        ...
        for (NodesGroup group : selectedGroups) {
            SearchUnit selectedNode = ingesterNodeSelector.selectNodeFromGroup(group, shardIdStr, indexName, usedZones);
            ...
            if (selectedNode != null) {
                ...
                log.info(
                    "PRIMARY allocation for shard {}/{}: selected node {} from group {} (zone={})",
                    indexName,
                    shardId,
                    selectedNode.getName(),
                    group.getGroupId(),
                    selectedNode.getZone()
                );
            } else {
                log.warn("PRIMARY allocation for shard {}/{}: no node selected from group {}", indexName, shardId, group.getGroupId());
            }
        }

        if (selectedNodes.isEmpty()) {
            log.error("PRIMARY allocation for shard {}/{}: no nodes selected. Allocation failed.", indexName, shardId);
        }
    ...
    private List<SearchUnit> validateExistingIngesters(List<String> ingesterNames, List<SearchUnit> allNodes) {
        ...
        for (String nodeName : ingesterNames) {
            SearchUnit node = allNodes.stream().filter(n -> n.getName().equals(nodeName)).findFirst().orElse(null);
    ...
    private List<SearchUnit> getReplicaNodesForAllocation(
        ...
        log.debug(
            "REPLICA allocation for shard {}/{}: current groups = {}, desired groups = {}",
            indexName,
            shardId,
            currentGroupCount,
            desiredGroupCount
        ...
        if (currentGroupCount == desiredGroupCount && !currentGroupIds.isEmpty()) {
            ...
            log.info(
                "Shard {}/{} already allocated to {} groups - stable allocation, returning current nodes",
                indexName,
                shardId,
                currentGroupCount
            );
        ...
        if (currentGroupCount > desiredGroupCount) {
            log.warn(
                "Shard {}/{} currently allocated to {} groups but config requires {} groups. "
        ...
        log.info(
            "Shard {}/{} needs scale-up: current = {} groups, desired = {} groups",
            indexName,
            shardId,
            currentGroupCount,
            desiredGroupCount
        ...
        if (allGroups.isEmpty()) {
            log.warn("No groups discovered for shard {}/{}. Cannot proceed with group-based allocation.", indexName, shardId);
            return List.of();
        }
        ...
        if (replicaGroups.isEmpty()) {
            log.warn("No REPLICA groups found for shard {}/{}. Cannot proceed with group-based allocation.", indexName, shardId);
            return List.of();
        }
        ...
        if (availableGroups.isEmpty()) {
            log.warn("No available REPLICA groups for scale-up of shard {}/{} (all groups already allocated)", indexName, shardId);
        ...
        if (additionalGroups.isEmpty()) {
            log.warn("No additional groups selected for shard {}/{} scale-up", indexName, shardId);
            // Return nodes from current groups
            return getNodesFromGroupIds(currentGroupIds, allNodes, shardId, indexName, targetRole);
        }

        log.info(
            "REPLICA scale-up for shard {}/{}: Adding {} new group(s) to existing {} group(s)",
            indexName,
            shardId,
            additionalGroups.size(),
            currentGroupCount
        );
     ...
     */
    private List<SearchUnit> getNodesFromGroupIds(
        Set<String> groupIds,
        List<SearchUnit> allNodes,
        int shardId,
        String indexName,
        ...
        for (SearchUnit node : allNodes) {
            ...
            log.trace("REPLICA stable allocation: node {} from group {} exists - keeping", node.getName(), node.getShardId());
        }

        if (eligibleNodes.isEmpty()) {
            log.warn("No nodes found in groups {} for shard {}/{} (nodes missing from discovery)", groupIds, indexName, shardId);
        } else {
            log.debug(
                "REPLICA stable allocation for shard {}/{}: returning {} nodes from {} groups",
                indexName,
                shardId,
                eligibleNodes.size(),
                groupIds.size()
