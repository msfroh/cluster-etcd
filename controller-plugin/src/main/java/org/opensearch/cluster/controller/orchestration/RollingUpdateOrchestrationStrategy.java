...
import org.opensearch.cluster.controller.models.IndexShardProgress;
...
import static org.opensearch.cluster.controller.metrics.MetricsConstants.ROLLING_UPDATE_PROGRESS_PERCENTAGE_METRIC_NAME;
import static org.opensearch.cluster.controller.metrics.MetricsConstants.ROLLING_UPDATE_TRANSIT_NODES_PERCENTAGE_METRIC_NAME;
import static org.opensearch.cluster.controller.metrics.MetricsUtils.buildMetricsTagsByRole;
...
public class RollingUpdateOrchestrationStrategy implements GoalStateOrchestrationStrategy {
    private static final Logger log = LogManager.getLogger(RollingUpdateOrchestrationStrategy.class);
    ...
    private void orchestrateNodeGroup(
        String indexName,
        String shardId,
        List<String> nodeGroup,
        NodeRole role,
        ShardAllocation planned,
        String clusterId
    ) {
        ...
        if (availableSlots > 0) {
            ...
            List<String> nextBatch = nodesToUpdate.stream().limit(batchSize).toList();

            log.info(
                "Starting batch update for {} {} nodes for shard {}/{}: {} nodes ({}% in transit)",
                role,
                nodeGroup.size(),
                indexName,
                shardId,
            ...
            log.info("Index-shard {} {} group has {}% nodes in transit, waiting for convergence", indexShard, role, transitPercent);
    ...
    private boolean hasGoalStateUpdated(String nodeId, String indexShard, String clusterId) {
        try {
            ...
            return goalState.getLocalShards().containsKey(indexName) && goalState.getLocalShards().get(indexName).containsKey(shardId);
        } catch (Exception e) {
    ...
    private boolean hasActualStateConverged(String nodeId, String indexShard, String clusterId) {
        try {
            ...
            return shards.stream().anyMatch(shard -> String.valueOf(shard.getShardId()).equals(shardId));
        } catch (Exception e) {
    ...
    private void updateNodeGoalState(String nodeId, String indexName, String shardId, ShardAllocation planned, String clusterId)
        throws Exception {
        try {
    ...
    private int cleanupNodeGoalState(String clusterId, String nodeId) throws Exception {
        ...
        for (Map.Entry<String, Map<String, String>> indexEntry : localShards.entrySet()) {
            ...
            for (Map.Entry<String, String> shardEntry : shards.entrySet()) {
                ...
                if (planned == null) {
                    ...
                    log.info("Removing stale goal state: node={}, index/shard={}/{} (no planned allocation)", nodeId, indexName, shardId);
                    removedCount++;
                    continue;
                }
                ...
                if (isInPlannedAllocation) {
                    ...
                    log.info(
                        "Removing stale goal state: node={}, index/shard={}/{}, role={} (not in planned allocation)",
                        nodeId,
                        indexName,
                        shardId,
                        role
        ...
        if (removedCount > 0) {
            ...
            log.debug("Updated goal state for node {} after cleanup: {} index/shard entries removed", nodeId, removedCount);
        }
